                ...
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead class="bg-bg text-amber-400 uppercase text-xs font-bold font-heading tracking-wider border-b border-amber-500/30">
                        <tr>
                            ...
                            <th class="p-4 rounded-r-xl text-center min-w-[390px]">DIRECT ACTIONS</th>
                            </tr>
                            </thead>
                            <tbody id="usersTableBody" class="divide-y divide-amber-500/20 text-neutral-200">
                                    ...
                                    <tr class="hover:bg-amber-500/10 transition user-row">
                                        ...
                                        <td class="p-4 min-w-[180px]">
                                            <div class="space-y-1">
                                                <div class="flex items-center gap-1.5">
                                                    ...
                                                    <button onclick="copyToClipboard('{{ $user->referral_code }}', 'Referral Code copied!')"
                                                        class="p-1 rounded hover:bg-amber-500/20 text-amber-400 transition" title="Copy Referral Code">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-amber-400" viewBox="0 0 24 24"
                                                ...
                                                <div class="flex items-center gap-1.5">
                                                    <button
                                                        onclick="copyToClipboard('{{ url('/user/register?sponsor=' . $user->referral_code . '&position=left') }}', 'Left Referral Link copied!')"
                                                        class="px-2 py-0.5 rounded bg-amber-500/20 text-amber-300 hover:bg-amber-500/40 text-[10px] font-bold font-mono border border-amber-500/40"
                                                        title="Copy Left Leg Link">
                                                        Copy Left
                                                    </button>
                                                    <button
                                                        onclick="copyToClipboard('{{ url('/user/register?sponsor=' . $user->referral_code . '&position=right') }}', 'Right Referral Link copied!')"
                                                        class="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/40 text-[10px] font-bold font-mono border border-emerald-500/40"
                                            ...
                                            <div>
                                                ...
                                                <div class="mt-1">
                                                    @if(strtolower((string) $user->position) === 'left')
                                                        <span
                                                            class="px-2 py-0.5 rounded bg-amber-500/20 text-amber-300 border border-amber-500/40 text-[9px] font-black uppercase">👈
                                                            LEFT LEG</span>
                                                    @elseif(strtolower((string) $user->position) === 'right')
                                                        <span
                                                            class="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 text-[9px] font-black uppercase">RIGHT
                                                            LEG 👉</span>
                                                    @else
                                                        <span
                                                            class="px-2 py-0.5 rounded bg-neutral-800 text-neutral-400 border border-neutral-700 text-[9px] font-bold uppercase">UNASSIGNED</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="p-4 min-w-[180px]">
                                            @if($activePkg)
                                                <div class="space-y-1">
                                                    <span
                                                        class="px-2.5 py-1 rounded bg-amber-500/20 text-amber-300 border border-amber-500/40 text-xs font-black uppercase font-heading block">
                                                        📦 {{ $activePkg->package->name ?? 'Active Plan' }}
                                                    </span>
                                                    <div class="text-xs font-mono font-bold text-emerald-400">
                                                        ${{ number_format($activePkg->invested_amount, 2) }} <span
                                                            class="text-[10px] text-neutral-400 font-normal">({{ $activePkg->daily_roi }}% Daily ROI)</span>
                                                    </div>
                                                </div>
                                            @else
                                                <span
                                                    class="px-2.5 py-1 rounded bg-neutral-800 text-neutral-400 border border-neutral-700 text-[10px] font-bold uppercase">
                                                    NO ACTIVE PACKAGE
                                                </span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="p-4 min-w-[140px]">
                                            ...
                                            @else
                                                <span
                                                    class="px-2.5 py-1 rounded bg-rose-500/10 text-rose-400 border border-rose-500/30 text-[10px] font-bold uppercase">NOT
                                                    ACTIVATED</span>
                                            @endif
                                        </td>

                                        <!-- Status (WITH LIVE BLINKING DOTS: GREEN FOR ACTIVE, RED FOR INACTIVE) -->
                                        <td class="p-4 min-w-[130px]">
                                            @if($user->status === 'active')
                                                <span
                                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 text-[10px] font-black uppercase shadow-[0_0_8px_rgba(16,185,129,0.2)]">
                                            ...
                                            @else
                                                <span
                                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded bg-rose-500/20 text-rose-300 border border-rose-500/40 text-[10px] font-black uppercase shadow-[0_0_8px_rgba(244,63,94,0.2)]">
                                                    <span class="relative flex h-2 w-2">
                                                        <span
                                        ...
                                        <td class="p-4 min-w-[140px]">
                                            @if($user->is_bot_active)
                                                <span
                                                    class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/20 border border-emerald-500/60 text-emerald-300 text-[10px] font-black uppercase tracking-wider animate-pulse shadow-[0_0_12px_rgba(16,185,129,0.5)]"
                                                    title="Activated at: {{ $user->bot_activated_at?->format('d M Y H:i') }}">
                                                    <span class="relative flex h-2 w-2">
                                                        <span
                                                            class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                                                        <span class="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                                            ...
                                            @else
                                                <span
                                                    class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-[10px] font-bold uppercase tracking-wider">
                                        ...
                                        <td class="p-4 min-w-[390px]">
                                            <div class="flex items-center justify-start gap-1.5 whitespace-nowrap">
                                                <!-- 1. DIRECT ADD FUND BUTTON -->
                                                <button onclick="openAddFundModal('{{ $user->id }}', '{{ $user->name }}', '{{ $user->referral_code }}')"
                                                    class="px-3 py-1.5 rounded-xl border border-amber-400 bg-amber-500 text-black font-black text-xs hover:bg-amber-400 transition flex items-center gap-1 shadow shrink-0"
                                                    title="Direct Add Fund to Wallet">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-black" viewBox="0 0 24 24" fill="none"
                                                ...
                                                <a href="{{ route('admin.users.impersonate', $user->id) }}" target="_blank"
                                                    class="p-2 rounded-xl border border-amber-500/60 bg-amber-500/10 text-amber-300 hover:bg-amber-500/30 transition shadow flex items-center justify-center shrink-0"
                                                    title="Login as User">
                                                ...
                                                <!-- 3. DIRECT MEMBERS VIEW WITH COUNT -->
                                                <a href="{{ route('admin.network.direct', ['search' => $user->referral_code]) }}"
                                                    class="px-2.5 py-1.5 rounded-xl border border-amber-500/60 bg-amber-500/10 text-amber-300 hover:bg-amber-500/30 transition shadow flex items-center gap-1.5 shrink-0"
                                                    title="View Direct Members (Total: {{ $user->directMembers->count() }})">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-amber-400" viewBox="0 0 24 24" fill="none"
                                                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                                    ...
                                                    <span class="text-xs font-black text-amber-300 font-mono">{{ $user->directMembers->count() }}</span>
                                                </a>
                                                ...
                                                <!-- 7. TOGGLE STATUS (SET ACTIVE / INACTIVE) -->
                                                {{-- <form action="{{ route('admin.users.toggle-status', $user->id) }}" method="POST"
                                                    class="inline-flex items-center shrink-0"
                                                    onsubmit="return confirm('Are you sure you want to {{ $user->status === 'active' ? 'deactivate' : 'activate' }} {{ addslashes($user->name) }}?');">
                                                    @csrf
                                                    <button type="submit"
                                                        class="p-2 rounded-xl border {{ $user->status === 'active' ? 'border-rose-500/60 bg-rose-500/10 text-rose-300 hover:bg-rose-500/30' : 'border-emerald-500/60 bg-emerald-500/10 text-emerald-300 hover:bg-emerald-500/30' }} transition shadow flex items-center justify-center shrink-0"
                                                        title="{{ $user->status === 'active' ? 'Deactivate Member (Set Inactive)' : 'Activate Member (Set Active)' }}">
                                                        @if($user->status === 'active')
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-rose-400" viewBox="0 0 24 24"
                                                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                                            stroke-linejoin="round">
                                                            <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" />
                                                            <circle cx="9" cy="7" r="4" />
                                ...
                                @empty
                                            ...
                                            <div id="addFundModal"
                                                ...
                                                <div
                                                    class="bg-panel border border-amber-500/40 rounded-2xl p-6 w-full max-w-md shadow-2xl space-y-5 relative animate-in fade-in zoom-in duration-200">
                                                    ...
                                                    <form id="addFundForm" action="" method="POST" class="space-y-4">
                                                        ...
                                                        <div class="space-y-1.5">
                                                            ...
                                                            <select name="wallet_type"
                                                                class="w-full px-4 py-3 rounded-xl bg-bg border border-amber-500/40 text-white font-bold text-xs focus:outline-none focus:border-amber-400 cursor-pointer"
                                                                required>
