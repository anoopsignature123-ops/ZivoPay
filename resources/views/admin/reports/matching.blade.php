...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-panel p-6 shadow-2xl rounded-2xl border border-amber-500/30">
        <div>
            ...
            <h1 class="text-2xl font-black font-heading text-white uppercase tracking-wider">
                Matching / Team Level Income Report
            </h1>
            <p class="text-xs text-neutral-400 mt-1">Monitor team matching and level commission distributions across member network structures</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-panel p-5 rounded-2xl border border-amber-500/30 shadow-xl flex items-center gap-4">
            ...
            <div>
                <p class="text-[11px] font-bold text-amber-400/80 uppercase">Total Matching Paid</p>
        ...
        <div class="bg-panel p-5 rounded-2xl border border-amber-500/30 shadow-xl flex items-center gap-4">
            ...
            <div>
                <p class="text-[11px] font-bold text-amber-400/80 uppercase">Total Matching Logs</p>
        ...
        <div class="bg-panel p-5 rounded-2xl border border-amber-500/30 shadow-xl flex items-center gap-4">
            ...
            <div>
                <p class="text-[11px] font-bold text-amber-400/80 uppercase">Matching Ratio</p>
                <p class="text-sm font-bold text-amber-300 mt-1">50:50 Team Matching Ratio</p>
            </div>
        </div>
    </div>
    ...
    <div class="p-3.5 rounded-2xl bg-bg/80 border border-amber-500/40 shadow-lg">
        <form action="{{ route('admin.reports.matching') }}" method="GET" class="flex flex-nowrap items-end gap-3 w-full overflow-x-auto text-xs font-sans pb-1">
            ...
            <div class="flex items-center gap-2 shrink-0">
                ...
                <a href="{{ route('admin.reports.matching') }}" class="py-2.5 px-4 rounded-xl bg-black/60 border border-white/60 text-white hover:bg-white/10 font-bold text-xs transition flex items-center justify-center shrink-0">
                    Reset
                </a>
            </div>
        </form>
                ...
                <tbody class="divide-y divide-amber-500/20 text-neutral-200">
                    ...
                    <tr>
                        <td colspan="8" class="p-8 text-center text-neutral-400 font-medium">
                            No Matching Income records found matching your filter parameters.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
