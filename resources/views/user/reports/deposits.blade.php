...
@section('title', 'Deposit Fund History - ZIVO PAY')
...
<div class="space-y-6 font-sans">
    ...
    <div class="p-6 sm:p-8 rounded-3xl bg-[#042718] border-2 border-emerald-500/60 shadow-[0_0_35px_rgba(16,185,129,0.25)] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-2xl sm:text-3xl font-black text-white uppercase tracking-tight mt-1">ADD FUND & DEPOSIT HISTORY</h1>
            <p class="text-xs text-neutral-300 mt-1">Audit log of all manual and automated Fund Wallet deposits.</p>
        </div>

        <div class="px-5 py-3 rounded-2xl bg-[#02180f] border border-emerald-500/40 text-right">
            <span class="text-[10px] text-emerald-400 font-extrabold uppercase tracking-wider block">Total Fund Wallet Balance</span>
    ...
    <div class="bg-[#042718] p-4 rounded-2xl border border-emerald-500/30">
        <form action="{{ route('user.reports.deposits') }}" method="GET" class="flex flex-wrap items-center gap-3">
            <div>
            ...
            <div class="pt-5 flex items-center gap-2">
                ...
                <a href="{{ route('user.reports.deposits') }}" class="px-3 py-2 bg-[#01140c] border border-emerald-500/30 text-neutral-400 text-xs rounded-xl hover:text-white transition">
                    Reset
                </a>
            </div>
        </form>
