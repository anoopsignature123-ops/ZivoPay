...
<div class="space-y-6 font-sans">
    ...
    <div class="p-4 rounded-2xl bg-[#042718] border border-emerald-500/30 shadow-xl">
        <form action="{{ request()->url() }}" method="GET" class="zivo-filter-bar">
            <!-- FROM DATE -->
            ...
            <div class="zivo-filter-actions">
                ...
                <a href="{{ request()->url() }}" class="py-2 px-3.5 rounded-xl bg-black/60 border border-emerald-500/30 text-neutral-300 font-bold text-xs hover:text-white hover:border-emerald-400 transition text-center shrink-0">
                    Reset
                </a>
            </div>
        </form>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-8 text-center text-neutral-400 font-bold">
                                No subscription referral income records found matching your filter.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
